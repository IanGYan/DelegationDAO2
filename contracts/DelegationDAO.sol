...
contract DelegationDAO is Ownable {
    ...
    // Schedule revoke, only Owner can call this function
    function schedule_revoke() public onlyOwner {
    ...
    // Change the collator target, owner only
    function change_target(address newCollator) public onlyOwner {
    ...
    // Reset the DAO state back to COLLECTING, owner only
    function reset_dao() public onlyOwner {
        currentState = daoState.COLLECTING;
    }
